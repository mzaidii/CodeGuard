...
def print_progress_bar(current, total, elapsed, bar_length=40):
    ...
    sys.stdout.write(f"\r🔍 Scanning: [{bar}] {percent*100:.1f}% ({elapsed:.1f}s)")
    sys.stdout.flush()

def review_code(code, show_progress=True):
    ...
    estimated_tokens = max(code_lines * 100, 500)  # Increased estimate
    ...
    try:
        ...
        if show_progress:
            # Final progress bar
            sys.stdout.write(f"\r🔍 Scanning: [{'█' * 40}] 100.0% ✅ Complete ({elapsed:.1f}s)\n")
            sys.stdout.flush()
